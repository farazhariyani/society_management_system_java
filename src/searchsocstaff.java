...
public class searchsocstaff extends JInternalFrame {
	...
	public void fillcombobox2()
	{
		...
		String query="select distinct STAFF_ID from SOCIETY_STAFF order by STAFF_ID ";
		PreparedStatement pst=con.prepareStatement(query);
		ResultSet rs=pst.executeQuery();
		...
		{	
			...
			c2.addItem(rs.getString("STAFF_ID"));
		}
		con.close();
		} catch (SQLException e) {
			// TODO Auto-generated catch block
	 ...
	 * Create the frame.
	 */
	public searchsocstaff() throws SQLException{
	  try
	  {
		  setTitle("Society Staff");
			...
			JLabel lblOwner = new JLabel("SOCIETY STAFF");
			lblOwner.setFont(new Font("Times New Roman", Font.BOLD, 20));
			lblOwner.setBounds(285, 11, 185, 24);
				...
				try{
					...
					*/
					...
					//if(rbtnoid.isSelected())
					{
						...
						String q1="select * from SOCIETY_STAFF where STAFF_ID="+c2.getSelectedItem();
						rs = stmt.executeQuery( q1 );
						//buildTableModel(rs);
						ResultSetMetaData md = rs.getMetaData();
						int columns = md.getColumnCount();
		 ...
		 delete.setIcon(new ImageIcon("F:\\DBProjectJan2017\\src\\delete.png"));
			delete.addActionListener(new ActionListener() {
				...
				try
				{
					...
					String query = "Delete from SOCIETY_STAFF where STAFF_ID="+value;
			...
			update.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					societystaff so=new societystaff(value);
