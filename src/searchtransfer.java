...
public class searchtransfer extends JInternalFrame {
	...
	public void fillcombobox2()
	{
		...
		String query="select distinct FLAT_ROWHOUSE_NO from TRANSFER order by FLAT_ROWHOUSE_NO ";
		PreparedStatement pst=con.prepareStatement(query);
		ResultSet rs=pst.executeQuery();
		...
		{	
			...
			c2.addItem(rs.getString("FLAT_ROWHOUSE_NO"));
		}
		con.close();
		} catch (SQLException e) {
			// TODO Auto-generated catch block
	 ...
	 * Create the frame.
	 */
	public searchtransfer() throws SQLException{
	  try
	  {
		  setTitle("TRANSFER");
			//setMaximum(true);
			...
			JLabel lblOwner = new JLabel("TRANSFER");
			lblOwner.setFont(new Font("Times New Roman", Font.BOLD, 20));
			lblOwner.setBounds(285, 11, 138, 24);
				...
				try{
					...
					*/
					...
					//if(rbtnoid.isSelected())
					{
						...
						String q1="select * from TRANSFER where FLAT_ROWHOUSE_NO="+c2.getSelectedItem();
						rs = stmt.executeQuery( q1 );
						//buildTableModel(rs);
						ResultSetMetaData md = rs.getMetaData();
						int columns = md.getColumnCount();
		 ...
		 delete.setIcon(new ImageIcon("F:\\DBProjectJan2017\\src\\delete.png"));
			delete.addActionListener(new ActionListener() {
				...
				try
				{
					...
					String query = "Delete from TRANSFER where FLAT_ROWHOUSE_NO="+value;
			...
			update.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					{
						int row = table.getSelectedRow();
						int value = Integer.parseInt(table.getModel().getValueAt(row, 2).toString());
					transfer so=new transfer(value);
